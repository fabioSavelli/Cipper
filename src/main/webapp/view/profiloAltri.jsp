	...
	<div class="container-fluid" id="sfondo">
		<div class="container">
			...
			<div class="col-lg-4 offset-lg-1">
				<h2 class="row font-weight-light">
					...
					<img src='<s:property value="altro.foto" />' id="fotoUtente" line-height="0px" width="50" height="50" class="d-inline-block align-top" alt=""></a> 
					<a href="#">@<s:property value="altro.username"/></a>
				</h2>
				<br>
					...
					<div class="modal-dialog modal-lg" role="document">
						<div class="modal-content">
							<div class="modal-header">
								...
								<div class="modal-body">
								<img src='<s:property value="altro.foto" />' line-height="0px" width="100%" height="100%" class="d-inline-block align-top" alt=""></a>
								</div>
						</div>
					</div>
				</div>
			...
			<div class="col-lg-4 offset-lg-2">
				...
				<s:set value='altro.idUtente' var="idAltro"></s:set>
				...
				Utente altro 	 		= lm.read((Integer)pageContext.getAttribute("idAltro"));
			...
			%>
				<s:form action="segui">
					<input type="hidden" name="idUtente"
						value='<% out.print(utente.getIdUtente()); %>'>
					<input type="hidden" name="idAltro"
						value='<% out.print(altro.getIdUtente()); %>'>

					<button type="submit"
						class="btn <% out.print(classe); %> btn-lg btn-block font-weight-light">
						<% out.print(segui); %>
					</button>
				</s:form>
			</div>
			...
			<nav>
				<div class="nav nav-tabs" id="nav-cip-tab" role="tablist">
					<a class="nav-item nav-link active col-5 offset-1"
						...
						aria-selected="true"> <font color=" #008250">i suoi Cip</font> </a> 
					...
					<a class="nav-item nav-link col-5" id="nav-reCip-tab"
						...
						<font color=" #008250">i suoi ReCip</font> </a>
				</div>
			</nav>
			...
			<%
			ArrayList<Cip> altroCip 	= cm.getAllMyCip(altro.getIdUtente());
						
			if(altroCip.size()==0){
				
				out.println("<div class='alert alert-warning col-md-12 offset-md-1' role='alert' style='text-align: center;'>");
				out.println("ATTENZIONE! Questo utente non ha ancora pubblicato nulla sul proprio account.");
				out.println("</div>");
			}
			...
			for(i=0; i<altroCip.size(); i++){
				...
				//data
				out.println("<h5 class='card-subtitle mb-2 text-muted font-weight-light col-md-5'>"+cm.stampaData(attuale.getData())+"</h5>");
				...
				out.println("<form id='reCipAltri' name='reCipAltri' action='/cipperProva/reCipAltri.action' method='post'>");
				out.println("<input type='hidden' name='utente.idUtente' value='"+utente.getIdUtente()+"'>");
				out.println("<input type='hidden' name='altro.idUtente'  value='"+attuale.getIdAutore()+"'>");	
		...
		%>
				...
				<div class="tab-pane fade" id="nav-reCip" role="tabpanel" aria-labelledby="nav-reCip-tab">
					...
					ArrayList<ReCip> tuttiReCip = rcm.getAllMyReCip(altro);
					
					if(tuttiReCip.size() == 0){
						out.println("<div class='alert alert-warning col-md-12 offset-md-1' role='alert' style='text-align: center;'>");
						out.println("ATTENZIONE! Questo utente non ha effettuato alcun recip.");
						out.println("</div>");
					}
					...
					for(i=0; i<tuttiReCip.size(); i++){
						...
						out.println("<form id='reCipAltri' name='reCipAltri' action='/cipperProva/reCipAltri.action' method='post'>");
						out.println("<input type='hidden' name='utente.idUtente' value='"+utente.getIdUtente()+"'>");
						out.println("<input type='hidden' name='altro.idUtente'  value='"+altro.getIdUtente()+"'>");	
